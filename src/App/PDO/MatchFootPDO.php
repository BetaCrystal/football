...
// Instancie un MatchFoot depuis un tableau associatif + evite de répéter le code ex:t dans getById et getAll
    ...
    {
        try {
            $stmt = $pdo->prepare("INSERT INTO matches (team_score, opponent_score,
            date, city, team_id, opposing_club_id)
            VALUES (:team_score, :opponent_score, :date, :city, :team_id, :opponent_id)");
    ...
    {
        try {
            $stmt = $pdo->prepare("UPDATE matches SET team_score = :team_score,
            opponent_score = :opponent_score, date = :date, city = :city,
            team_id = :team_id, opposing_club_id = :opponent_id
            WHERE id = :id");
